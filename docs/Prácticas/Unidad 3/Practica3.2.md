...
Y tras ompletar la instalación ejecutaremos ```node -v``` para comprobar la version y que se ha instalado correctamente

![Imagen versión de node](../../assets/images/Practica3.2/version_node.png)

### Despliegue de una nueva aplicación

Vamos a realiza ahora el despliegue de una aplicación, para acelerar todo este proceso
...
Cuando ejecutemos este comando el servidor no arracanra y mostrara el siguiente error en consola:
...
2. Asegurarese de que el archivo package.json contiene la siguiente línea.
...
Y ya por ultimo podremos ejecutar ```npm run start```

![Npm start](../../assets/images/Practica3.2/npm_start.png)

Y como se puede ver el servidor ya esta en marcha y funcionando. Para conectarnos
en el navegador pondremos la ip de la maquina:puerto, en mi caso
```192.168.8.63:3000```
...
En el apartado anterior hemos visto cómo desplegar una aplicacion de Node.js sobre un servidor Express en local.Por lo tanto
podriamos decir que es una forma rápida para el desarrollador para ver los cambios que esta haciendo.
...
Funciona conectándose a un repositorio de GitHub, de donde extrae el código fuente. A continuación, ejecutrá
un proceso de construcción para pre-renderizar las páginas de nuestro sitio web/aplicación en archivos estaticos.
...
En ese directorio, crearemos los 3 archivos (dos .html y un .js) que confromarán nuestra sencilla aplicación de ejemplo:
...
Por ultimo probaremos que nuestra aplicación funciona perfectamente en local:
...
Una vez instalado esta claro que para realizar acciones de deploy, netlify nos solicitara una autenticacion, esto se hace mediante
el comando:
...
Y como podemos ver, si accedemos al panel de contro de netlify y navegamos por el, podremos acceder a nuestro sitio web

![Sitio web desplegado en netlify](../../assets/images/Practica3.2/Sitio_web.png)

#### Despliegue mediante conexión con Github

En primer lugar, vamos a elminar el sitio web que hemos desplegado antes en Netlify para evitar cualquier problema o conflicto:

En segundo lugar, vamos a borrar el directorio dodne se halla el repositorio clonado en el paso anterior para así poder empezar
de 0:
...
Tras esto entramos en la carpeta donde está el codigo:
...
Por ultimo sólo queda referenciar nuestra carpeta el repositorio recién creado en Github y hacer un un push para subir todo el contenido
...
Y seleccionamos que no acceda a todos nuestros repositorios sino sólo al repositorio que necesitamos, que es
...
![Confirmacion despliegue](../../assets/images/Practica3.2/variable_de_entorno.png)
...
URLs del sitio pueden acceder. A este archivo se puede acceder a través de la URL del site:
